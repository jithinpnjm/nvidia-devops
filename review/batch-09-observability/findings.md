...
## docs/nvidia-zero-to-hero/volume-17/labs

### lab-01-placeholder.md (Profiling Fundamentals)
- No findings. Realistic PyTorch profiler + Nsight Systems walkthrough with plausible expected output, sensible profiler-comparison table (overhead, granularity, use case), good troubleshooting section.

### lab-02-placeholder.md (Roofline Analysis with Nsight Compute)
- [SEVERITY: high] Systematic unit-conversion error in the roofline crossover-point and prediction arithmetic, in addition to inheriting Ch.03's wrong "141 TFLOPS" H100 peak. Exercise 3 computes "Crossover point: 141 TFLOPS / 2000 GB/s = 0.0705 FLOPS/byte" (line 130) — dividing TFLOPS by GB/s without converting units consistently drops a factor of 1000; done correctly (141×10¹² FLOPS/s ÷ 2000×10⁹ bytes/s), the crossover is 70.5 FLOPS/byte, not 0.0705 — and this exact lab's own volume (Ch.03) elsewhere states the correct 70.5 FLOPS/byte crossover for the same H100/141TFLOPS pairing, so this lab directly contradicts its own volume's chapter content by a factor of 1000. The same unit-dropping bug repeats in the elementwise-kernel roofline prediction: "0.25 × 2000 GB/s = 500 TFLOPS" (line 146) — 0.25 FLOPS/byte × 2000×10⁹ bytes/s = 500×10⁹ FLOPS/s = 0.5 TFLOPS, not 500 TFLOPS (again off by 1000x). The Python plotting code (Exercise 4) partially self-corrects this by dividing by 1000 in the roofline curve formula (`ci * peak_bw / 1000`, line 164) but the worked-example prose above it does not, so the lab is internally inconsistent about whether the conversion factor has been applied.
  - Evidence: line 130 (crossover 0.0705 vs Ch.03's correct 70.5 for the same GPU/peak pairing); line 146 ("500 TFLOPS" prediction that should be 0.5 TFLOPS); line 164 (plotting code does include the correct `/1000` conversion, contradicting the prose above it).
