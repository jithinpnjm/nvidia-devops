...
### chapter-11-system-design-inference-serving.md — ROOT-CAUSE REQUEST-RATE ERROR (~1000x) INVALIDATES NEARLY ALL DOWNSTREAM SIZING AND COST FIGURES

- [SEVERITY: high] Phase 1's peak-request-rate calculation is wrong by roughly 1000x, and every subsequent GPU-sizing, hardware-cost, and revenue/margin calculation in the chapter is built on this number. The chapter states: "Peak: 50K concurrent users × 5 requests/user/hour ÷ 3600 = **70K req/sec peak**." The correct arithmetic: 50,000 users × 5 requests/hour = 250,000 requests/hour; 250,000 ÷ 3,600 seconds/hour ≈ **69.4 requests/sec** — not 70,000 req/sec. The stated figure appears to mistake the computed ~69 for "70K" (appending three extra orders of magnitude).
  - Evidence: "Peak: 50K concurrent users × 5 requests/user/hour ÷ 3600 = 70K req/sec peak."
  - Why it matters for JR2018680: this single number is the seed value for the entire rest of the chapter. "70K req/sec × 300 tokens/req = 21M tokens/sec peak" (should be ≈69.4 × 300 ≈ 20.8K tokens/sec — also off by ~1000x); the GPU fleet sizing in Phase 3 ("252 L40S for peak," "$3M hardware") is sized against the inflated 21M tokens/sec figure and would be roughly 1000x too large against the corrected ~20.8K tokens/sec peak; and the entire Phase 5 revenue/margin analysis ("75.6B tokens/hour peak," "$4,233/day," "-$3.95M/year LOSS") inherits the same inflation. A workload of 50,000 concurrent users generating 5 requests/hour each (≈69 req/sec) is a modest, easily-served workload — the chapter's conclusion that it requires a 252-GPU, multi-million-dollar fleet running at a multi-million-dollar annual loss does not hold once the root request rate is corrected.
