...
### chapter-03-mig-profiles-and-placement.md
- [SEVERITY: high] The "Concrete sizing example" worked capacity-planning arithmetic is internally inconsistent and physically impossible, in a section explicitly designed to model quantitative rigor. Chapter 2 already established the hard constraint "you cannot exceed 100% compute or 80GB memory in aggregate" on one H100 — this chapter's own worked example violates it while marking itself correct (✓).
  - Evidence: lines 119-134. The layout "4×(1g.10gb) + 2×(3g.40gb)" is marked `✓` ("80GB used exactly") but: (a) memory sums to 4×10GB + 2×40GB = 40GB + 80GB = **120GB**, exceeding the H100's 80GB physical HBM by 50%; (b) compute-slice count sums to 4×1 + 2×3 = **10 slices**, but MIG on H100 provides only **7** total compute slices (as chapter 2's own profile table implies with "max 7 concurrent instances" of `1g`), not the "14 SMs" this chapter invents as the denominator. The immediately preceding rejected layout ("7×(1g.10gb) + 2×(3g.40gb) = oversubscribed ❌") correctly identifies oversubscription using the same math that should have flagged the "accepted" layout too.
