...
### chapter-06-mig-configuration-multi-tenant.md
- [SEVERITY: high] RECURRENCE of the wrong-MIG-facts pattern (previously flagged for A100 across other volumes; here it's H100, in the volume's dedicated MIG capstone). The "H100 MIG modes" table is fabricated and doesn't match real NVIDIA MIG profile names or accounting:
  - Real H100 80GB MIG profiles are `1g.10gb`, `1g.20gb`, `2g.20gb`, `3g.40gb`, `4g.40gb`, `7g.80gb` (the `7g` prefix always denotes the *full GPU* — for an 80GB H100 that's `7g.80gb`, not `7g.40gb`/`7g.20gb`/`7g.10gb` as the doc lists). The doc's profile names (`7g.40gb`, `7g.20gb`, `7g.10gb`, `1g.5gb`) don't correspond to any real H100 MIG configuration.
  - MIG "compute units" are fractions out of **7** total GPU compute instances (e.g., 1g = 1/7, 3g = 3/7, 7g = 7/7), not "X/14 SMs" as the table states (`7g.40gb: 3/14 SMs` is internally contradictory — a "7g" profile claiming only 3 of 14 units is not the full-GPU profile "7g" is supposed to represent).
  - The claimed "H100 (80 GB HBM, **528 TensorCore TFLOPS**, ~2 TB/s memory bandwidth)" doesn't match any published H100 SXM5 figure (real: BF16 dense ≈989 TFLOPS, TF32 dense ≈495 TFLOPS, memory bandwidth ≈3.35 TB/s for SXM5, not ~2 TB/s — 2 TB/s is closer to the PCIe variant, but the doc later explicitly configures "H100 SXM5" hardware, line ~227).
  - Evidence: Line ~29 (Problem Statement GPU spec), ~38-45 (MIG profile table), ~227 (`nvidia-smi` output confirms "H100 SXM5").
  - Why it matters for JR2018680: This directly matches the explicitly-flagged "A100 MIG facts (SM count, max instances, profile memory) found wrong" pattern from this review series — here recurring for H100 in the exact capstone chapter dedicated to teaching MIG partitioning, which would be devastating if memorized ahead of an NVIDIA GPU-sharing interview question.
  - Suggested fix: Replace the MIG profile table with real H100 80GB profiles (1g.10gb / 1g.20gb / 2g.20gb / 3g.40gb / 4g.40gb / 7g.80gb), fix the compute-unit denominator to /7 (not /14), and correct the GPU headline spec to match H100 SXM5 (≈989 TFLOPS BF16 dense, ≈3.35 TB/s memory bandwidth) or explicitly use PCIe variant specs consistently if that's the intended SKU.

### chapter-07-kubernetes-gpu-scheduling.md
- [SEVERITY: medium] Fractional GPU resource requests (`nvidia.com/gpu: "0.5"`, `"0.25"`) are presented as if they work out-of-the-box with the standard Kubernetes device-plugin resource model, but the stock NVIDIA device plugin's `nvidia.com/gpu` extended resource is **integer-only** — you cannot request 0.5 or 0.25 of a GPU without additional configuration (NVIDIA GPU Operator time-slicing config, MPS, or a fractional-GPU scheduler extension). The YAML throughout the chapter (training-job.yaml, inference-job.yaml, research-job.yaml) uses fractional values as plain resource requests without noting this prerequisite.
  - Evidence: Line ~112-114, ~136-138 (`resources.requests.nvidia.com/gpu: "0.5"` / `"0.25"`).
...
### chapter-09-capacity-planning-forecast.md
- [SEVERITY: high] Foundational historical data is internally inconsistent by ~15x. The Historical Demand Data table states Q1 2024: "120 GPU-hrs/week" demand on a 16-GPU cluster with "68%" peak utilization. But 16 GPUs × 168 hrs/week × 0.68 utilization = **≈1,828 GPU-hours/week**, not 120 (off by ≈15x). The "120 GPU-hrs/week" figure would correspond to only ≈4.5% utilization of a 16-GPU cluster, not the stated 68%.
  - Evidence: Line ~45 (Historical Demand Data table, Q1 2024 row) vs the GPU-count/utilization math used throughout (`gpu_needs = demand / (utilization_target * 168 * 13)` and the manual walkthrough "Q0: 120/(170*0.85)=16 GPUs" at line ~336, which itself only works out to ≈0.83, not 16 — a separate, compounding arithmetic error in the same walkthrough step).
  - Why it matters for JR2018680: This is the seed data for the entire capacity-planning forecast (the capstone's core deliverable); if the baseline demand/utilization numbers don't reconcile with each other, every downstream GPU count, cost, and SLO conclusion in the chapter inherits the error.
  - Suggested fix: Recompute a self-consistent baseline — either the 68% utilization or the 120 GPU-hrs/week figure needs to change so that `GPU-hrs/week ≈ GPU_count × 168 × utilization` holds, and fix the "120/(170×0.85)=16" arithmetic in the Solution Walkthrough (which doesn't evaluate to 16 as written).

### chapter-10-training-cluster-design.md
- [SEVERITY: high] FLAGSHIP RECURRENCE of the 1000x unit-magnitude-slip pattern in the capstone's core sizing calculation (Step 1). "Total FLOPs per year: 100T × 100G = 10^22 FLOPs" is arithmetically wrong: 100 trillion (10^14) × 100 GFLOP (10^11) = **10^25 FLOPs**, not 10^22 (off by exactly 1000x). This directly feeds "GPUs needed: 10^22 / (4.42 × 10^19) ≈ 226 GPU-years" — using the correct 10^25 figure instead gives **≈226,244 GPU-years**, a 1000x larger and completely different (and unaffordable-on-$5M) answer.
  - Evidence: Line ~49 ("100T × 100G = 10^22 FLOPs"), ~54 ("GPUs needed: 10^22 / (4.42 × 10^19) ≈ 226 GPU-years").
  - Interesting note: Step 4 of the same chapter independently re-derives cluster capacity a different way (tokens/hour/GPU from TFLOPS) and arrives at a self-consistent, plausible "90 GPUs achieves 226T tokens/year," which happens to mask the Step 1 error rather than expose it — but Step 1's stated "10^22" and "226 GPU-years" arithmetic is still wrong on its own terms, and if a candidate walked through Step 1's logic literally in an interview (as its "spoken answer" section does, restating "10^22 FLOPs total needed... 230 GPU-years"), they would be reciting an internally inconsistent derivation.
  - Why it matters for JR2018680: This is the volume's flagship system-design capstone ("design a 100-GPU cluster from scratch"), explicitly named in the task brief as one of the highest-risk chapters for exactly this error class; a candidate who memorized the "100T × 100G = 10^22" step would fail a live "walk me through the math" follow-up.
  - Suggested fix: Correct the total-FLOPs figure to 10^25 in Step 1 and the interview-answer narrative, and reconcile it with Step 4's independently-correct 90-GPU/226T-tokens-per-year derivation — the two calculation paths should agree once the exponent is fixed (note: reconciling them properly would reveal the "100 trillion tokens/year" requirement is unrealistic for a $5M/90-GPU cluster using the Step 1 methodology, which is worth flagging as a design tension rather than silently glossing over).

### chapter-11-inference-serving-design.md
- [SEVERITY: low] Cost-per-request derivation has a mislabeled denominator: "$200K / (5 years × 365 days) = $110/day = $0.0046/request (at 1000 req/day)" — $110/day ÷ 1000 req/day = $0.11/request, not $0.0046. The $0.0046 figure is only consistent with ≈24,000 req/day (i.e., 1000 req/**hour**, matching Model A's stated rate), suggesting "1000 req/day" is a mislabeled "1000 req/hour."
  - Evidence: Line ~112.
  - Why it matters for JR2018680: Minor unit-label slip in the cost-per-request derivation that underlies the chapter's central "$0.001 budget" analysis; worth a quick fix for internal consistency.
  - Suggested fix: Relabel as "1000 req/hour → 24,000 req/day" to match the $0.0046 result.
- Core FLOPs/latency math (16.1T FLOPs, 11.5ms/request at 1400 TFLOPS, batching linearity) was verified correct. Otherwise reasonable design content with real vLLM/batching code.

### chapter-12-research-infrastructure-design.md
- [SEVERITY: high] Cluster utilization claim in the capstone's final "Fairness Verification" table is wrong by ~12.8x. "Cluster utilization: 768 GPU-hours / (32 GPUs × 14 days × 24 hours) = 91%" — verified: 32×14×24 = 10,752 total GPU-hours available over 2 weeks; 768/10,752 = **7.1%**, not 91%. For 91% utilization to be correct, total allocated GPU-hours would need to be ≈9,784, not 768 (the table's own per-group breakdown sums to exactly 768.0, so the "91%" is inconsistent with the table's own totals, not just a rounding issue).
  - Evidence: Line ~140-150 (Fairness Verification table and utilization line).
  - Why it matters for JR2018680: This is the capstone's final validation metric — the number that's supposed to prove the design meets the ">90% utilization" success criterion (line ~38). The design is presented as verified-successful using an 8x-wrong utilization figure; the real workload as specified (Group A alone: 2 jobs/week × 8 GPU × 48h = 768 GPU-hrs/week ≈ 1,536/2wk, and other groups similarly small) would actually yield low double-digit utilization, not the 90%+ target, meaning the design as specified likely does NOT meet its own success criterion.
  - Suggested fix: Either the group workload volumes (jobs/week × GPUs × duration) need to be increased substantially to actually fill 90%+ of a 32-GPU cluster over 2 weeks, or the utilization formula/claim needs to be corrected to reflect the true (much lower) achieved utilization — which would then require revisiting the "no starvation, 91% utilization" success narrative.
