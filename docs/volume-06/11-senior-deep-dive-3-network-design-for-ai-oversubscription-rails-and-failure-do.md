...
```
        ┌────────┐              ┌────────┐
        │ spine1 │              │ spine2 │        4 uplinks/leaf × 200Gb/s
        └───┬────┘              └───┬────┘        × 4 leaves = 3.2Tb/s
    ┌────────┼────────┐   ┌─────────┼────────┐    pod-to-pod bisection
    │        │        │   │         │        │
 leaf1-4 (Pod A, all nodes)     leaf1-4 (Pod B, all nodes)
    │        │        │   │         │        │
...
```
Rack / leaf switch X
  ├── GPU node (job's data-parallel replica)     ◀── scheduler placed both here
  └── Storage node (job's checkpoint replica)     ◀── for locality/low-latency

Leaf X fails ──▶ BOTH the live job AND its recovery path are gone simultaneously

Correct placement:
Rack / leaf X: GPU node (replica)          Rack / leaf Y: checkpoint replica
  Leaf X fails ──▶ live job lost, but checkpoint survives on an independent failure domain
```
