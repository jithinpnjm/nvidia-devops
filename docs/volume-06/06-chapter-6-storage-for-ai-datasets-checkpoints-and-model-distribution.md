...
│  Local NVMe   │   hot shards, checkpoint staging, decode cache
└──────┬────────┘
       │ async flush / burst write
       ▼
┌───────────────────────┐   shared, durable, concurrent-access
│ Parallel filesystem /  │   dataset shards, active checkpoint tier,
│ high-perf shared store │   model artifacts pulled at startup
└──────┬─────────────────┘
       │ lifecycle / cold tier
       ▼
┌───────────────┐   cheapest, highest latency
│ Object store  │   long-term checkpoint retention, dataset archive
└───────────────┘

Checkpoint burst path:  GPU HBM → local NVMe (fast absorb) → parallel FS (durable) → object store (archive)
```
