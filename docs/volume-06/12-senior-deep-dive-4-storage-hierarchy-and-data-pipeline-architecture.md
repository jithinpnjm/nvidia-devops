...
flowchart TD
  ...
  n0["throughput/IOPS (per GB) durability capacity ($/GB)"]
  n1["Local NVMe highest lowest smallest, cheapest per node"]
  n2["node-local failure domain"]
  n3["Parallel FS / high, shared durable mid-size, mid-cost"]
  n4["shared store across many clients (replicated/ shared failure domain"]
  n5["erasure-coded)"]
  n6["Object store lowest per-op highest largest, cheapest per GB"]
  n7["(higher latency, durability"]
  n8["great for fan-out) (multi-AZ/region)"]
```
...
flowchart LR
  %% Converted from the original ASCII diagram; source wording is preserved.
  n0["Node dies/is drained"]
  n1["local NVMe cache/scratch on that node is GONE, instantly, no replication"]
  n2["anything ONLY on local NVMe (not yet flushed to durable shared storage)"]
  n3["is lost — this includes in-flight checkpoint writes, unflushed logs,"]
  n4["and any 'scratch' preprocessing output the next stage depends on"]
  n0 --> n1
```
