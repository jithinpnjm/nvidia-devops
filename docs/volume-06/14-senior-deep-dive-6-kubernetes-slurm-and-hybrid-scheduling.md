...
```mermaid
flowchart TD
  Fleet["shared physical fleet"] --> Slurm["Slurm pool: batch"]
  Fleet --> K8s["Kubernetes pool: services"]
  Slurm <-->|"node moves between pools"| K8s
  Slurm -.-> Check["On every transition, verify RDMA/SR-IOV configuration is reprovisioned correctly"]
  K8s -.-> Check
```
