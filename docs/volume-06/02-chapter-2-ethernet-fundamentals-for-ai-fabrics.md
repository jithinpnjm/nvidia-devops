...
```
traffic increases ──▶ queue depth rises
                          │
                          ▼
                 ┌─────────────────┐
                 │  ECN marking     │  ecn_marked_packets↑, rx_dropped=0
                 │  (mark, don't    │  ← healthy: sender backs off before pain
                 │   drop)          │
                 └────────┬────────┘
                          │ still congested?
                          ▼
                 ┌─────────────────┐
                 │  PFC pause       │  rx_pause_ctrl_phy↑, prio3 pause duration↑
                 │  (stop upstream  │  ← fabric is intervening, not yet lossy
                 │   sender)        │
                 └────────┬────────┘
                          │ still congested?
                          ▼
                 ┌─────────────────┐
                 │  tail drop       │  rx_dropped/tx_dropped↑
                 │  (packet loss)   │  ← worst case: RDMA retransmit/timeout
                 └─────────────────┘
```
