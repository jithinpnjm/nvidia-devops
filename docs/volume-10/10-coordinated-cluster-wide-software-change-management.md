...
```mermaid
flowchart LR
  Lab["lab / reproduction"] --> Canary["representative canary"] --> Domain["limited failure-domain wave"] --> Fleet["fleet waves"]
  Evidence["At every step: evidence · compatibility · blast-radius check · scale"] -.-> Lab
  Evidence -.-> Canary
  Evidence -.-> Domain
  Evidence -.-> Fleet
```
