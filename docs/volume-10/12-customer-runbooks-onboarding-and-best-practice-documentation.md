...
VERIFICATION
  ...
  window → this runbook does NOT apply, see the maintenance-window runbook
  instead. If "Reason" names a kill-task failure, low memory, or a health-
  check script failure → continue below.

DECISION TREE
  ┌─ Is there a job still shown RUNNING on this node in `squeue -w gpu-node-041`?
  │
  ├─ YES → do NOT force-resume the node. Confirm via `nvidia-smi` on the node
  │         (SSH or `pdsh`) whether the GPU is actually healthy.
  │         ├─ nvidia-smi hangs or times out  → STOP, ESCALATE (see below) —
  │         │    do not attempt further mitigation on a node whose GPU driver
  │         │    is unresponsive; escalation owns physical/BMC-level recovery.
  │         └─ nvidia-smi returns normally, GPUs show no ECC/Xid errors →
  │              the drain reason was a stale kill-task failure; proceed to
  │              MITIGATION step A.
  │
  └─ NO (no job running) → proceed to MITIGATION step B.

MITIGATION
  A. (job appears healthy despite stale drain reason)
     $ scontrol update nodename=gpu-node-041 state=resume
     Expect: `sinfo` shows the node back in `idle` or `alloc` within ~30s.
     If the node re-enters `drain` automatically within 5 minutes with the
     SAME reason string → STOP, ESCALATE.

  B. (no job running, node genuinely idle)
     $ scontrol update nodename=gpu-node-041 state=resume
     Expect: node returns to `idle`. Monitor `sinfo` for 10 minutes; if a
     new job lands and completes normally, close as resolved.

ESCALATION
  Trigger: nvidia-smi unresponsive on the node, OR the node re-drains with
  an identical reason string within 5 minutes of a resume attempt.
  Contact: platform on-call (#gpu-fleet-oncall), page via PagerDuty
  service "gpu-cluster-hw". Do not attempt a second resume before paging.

POST-INCIDENT FOLLOW-UP
  Attach: `scontrol show node` output at time of drain, `sinfo` history,
  and whether this node has drained for the same reason in the last 30 days
  (recurring drain on one node is a hardware-suspect signal, not a fluke).
```
...
```
  ...
  ┌───────────────┐                    ┌─────────────────────┐
  │  "node stuck   │                    │  "here's how the     │
  │   in DRAIN"    │◀── referenced by ──│   cluster works, and  │
  │  exact steps   │    (not copied)    │   here's WHERE to     │
  │  for THIS      │                    │   look when something │
  │  symptom       │                    │   goes wrong"         │
  └───────────────┘                    └─────────────────────┘
