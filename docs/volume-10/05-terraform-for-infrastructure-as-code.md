...
## Start here: Terraform’s job and its safety boundary

Terraform declares API-managed objects such as networks, DNS records, IAM roles, cloud instances, object stores and Kubernetes resources. A provider translates Terraform operations into an external API. A resource is managed; a data source is read. Terraform state records which real object belongs to each configuration address so that a plan can compare configuration, state and the provider’s current observation.

The normal workflow is **format → validate → plan → review → apply → verify**. A plan is a proposal, not proof that the workload is healthy. Review `-` and `-/+` actions, identity/network/storage changes, replacement triggers, state sensitivity and blast radius before approval. Store team state in a protected remote backend with locking and recovery; never commit production state to Git.
