...
4. **Soak**: run `nvidia-smi`, `dcgmi diag` (NVIDIA Data Center GPU Manager's diagnostic CLI — runs tiered GPU self-tests from a quick sanity check up to a sustained-load stress test, catching a driver/VBIOS regression `nvidia-smi` alone wouldn't surface), and a representative multi-GPU NCCL/all-reduce job (NCCL — NVIDIA Collective Communications Library — the topology-aware library that does the actual fast GPU-to-GPU data movement for distributed training's gradient-sync step) on the canary nodes for at least one full job-length cycle — a driver regression that only shows up under sustained multi-hour collective communication load will not show up in a five-minute smoke test.
