...
```
Whole-GPU per tenant:
┌───────────────┐   ┌───────────────┐
│ Tenant A GPU0  │   │ Tenant B GPU1  │   ← separate physical devices,
│ (all SMs, all   │   │ (all SMs, all  │     no shared failure domain
│  memory)         │   │  memory)        │
└───────────────┘   └───────────────┘

MIG (hardware-partitioned, one physical GPU):
┌─────────────────────────────────────────┐
│ physical GPU                              │
│ ┌───────────────┐ ┌───────────────┐       │
│ │ Tenant A slice │ │ Tenant B slice │       │  ← separate SM/memory
│ │ (fixed SMs +    │ │ (fixed SMs +   │       │     slices, hardware fault
│ │  fixed memory)   │ │  fixed memory)  │       │     containment between them
│ └───────────────┘ └───────────────┘       │
└─────────────────────────────────────────┘

Time-slicing (software-scheduled, one physical GPU):
┌─────────────────────────────────────────┐
│ physical GPU — same SMs, same memory      │
│  A ▓▓▓▓░░░░B▓▓░░░░░░A▓▓▓▓▓▓░░░░B░░░░       │  ← time-sliced turns, no
│  ↑ scheduler decides who runs when,        │     isolation of memory or
│    a burst from A can starve B's turn       │     of "how long is my turn"
└─────────────────────────────────────────┘
```
