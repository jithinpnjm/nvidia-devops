...
Client request
      ...
      ▼
┌───────────────┐
│  Router/gateway │  ← must track which worker in each pool
│  (state-aware)   │    owns this sequence, not just round-robin
└───────┬───────┘
        │  1. route to prefill pool
        ▼
┌───────────────┐
│  Prefill pool   │  compute prompt → produce KV cache for the sequence
└───────┬───────┘
        │  2. KV cache transfer (NVLink on-node / RDMA cross-node)
        ▼
┌───────────────┐
│  Decode pool    │  holds KV state, emits tokens one at a time
└───────┬───────┘
...
```
Short prompt, short output (e.g. chatbot turn):
  prefill work  ▓▓                decode work  ▓▓▓
  → similar magnitude → little imbalance to exploit → AGGREGATE

Long prompt, short output (e.g. document Q&A):
  prefill work  ▓▓▓▓▓▓▓▓▓▓        decode work  ▓▓
  → prefill dominates → separate pool sized for compute → DISAGGREGATE

Short prompt, long output (e.g. long-form generation):
  prefill work  ▓▓                decode work  ▓▓▓▓▓▓▓▓▓▓
  → decode dominates → separate pool sized for KV/bandwidth → DISAGGREGATE
```
