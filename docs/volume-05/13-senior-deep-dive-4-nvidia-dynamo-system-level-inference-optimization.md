...
```
┌─────────────────────────────────────────────────────────┐
│ Dynamo (system-level, coordinates the distributed whole)  │
│  request routing (KV-aware) │ KV cache management          │
│  disaggregated serving       │ data transfer (RDMA/NVLink)  │
│  scaling                     │ Kubernetes-native deployment │
└──────────────────┬─────────────────────┬──────────────────┘
                    ▼                     ▼
          ┌────────────────┐    ┌────────────────┐
          │ Prefill engine   │    │ Decode engine    │
          │ (executes forward,│    │ (executes decode, │
          │  optimizes ONE     │    │  optimizes ONE     │
          │  GPU/node's work)   │    │  GPU/node's work)   │
          └────────────────┘    └────────────────┘
```
