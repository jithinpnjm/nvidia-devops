...
```
 High-priority Pod is Pending, Filter phase found zero feasible nodes as-is
        │
        ▼
 Scheduler looks for a node where evicting SOME lower-priority Pod(s)
 would make it feasible
        │
        ▼
 Candidate victim(s) identified on a node ── check: would evicting them
 violate their OWN PodDisruptionBudget (minAvailable)?
        │
   ┌────┴────┐
   ▼         ▼
  YES,      NO, would violate PDB ──▶ this node/victim set rejected,
  safe to        try a different candidate node
  evict
   │
   ▼
 Would the freed-up capacity actually pass the pending Pod's OWN Filter
 predicates (topology spread, affinity, storage locality)?
   ┌────┴────┐
   ▼         ▼
  YES        NO ──▶ rejected, try another node
   │
   ▼
 Evict victim(s), bind pending Pod to the freed node
```
