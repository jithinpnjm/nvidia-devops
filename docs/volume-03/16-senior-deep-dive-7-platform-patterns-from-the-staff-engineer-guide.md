...
➕ **The bulkhead isolation-unit spectrum, cost vs blast radius, for GPU pools specifically** — ordered from cheapest/widest blast radius to most expensive/narrowest:

| Isolation tier | Configuration | Blast radius | Cost |
| --- | --- | --- | --- |
| Shared | One GPU pool, all tenants, time-sliced or MIG-shared per device | A noisy/misbehaving tenant can starve others sharing the same physical GPU — isolation is only as strong as MIG/time-slicing enforcement, not a hard boundary | Cheapest |
| Shared pool, MIG-partitioned | One GPU pool, tenants get dedicated MIG slices (hard memory/SM partition) | Shrinks to "this MIG slice," not "this GPU" | Low |
| Dedicated node pool per tenant/tier | Separate node pools (e.g. a dedicated pool per model-serving tier) | One tenant's own pool | Idle capacity per pool if utilization isn't shared |
| Dedicated GPU pool per model/tenant | Full physical isolation | A bad deploy or driver issue in one pool cannot affect any other tenant's capacity at all | Most expensive |
