...
* Model State Memory (FP16/FP32 Adam): `$70B \times 12  bytes \approx 840  GB$`
* State per GPU (N=8): `$840  GB / 8 = 105  GB$`

`$105  GB > 80  GB$`. A 70B model cannot be trained on a single 8-GPU node with FSDP alone without CPU offloading or quantized optimizer states. Production deployments typically scale this across 2-4 nodes (16-32 GPUs) to fit comfortably.
