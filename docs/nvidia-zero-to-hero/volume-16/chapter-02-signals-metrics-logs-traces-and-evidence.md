...
## Beginner's Primer: The 3 Pillars of Observability

When troubleshooting an AI server, you will be bombarded with data. If you don't know *what* kind of data you are looking at, you will drown in it. 

There are exactly 3 pillars of observability. Every tool fits into one of these buckets:
1. **Metrics (The "What"):** These are numbers over time. *Example: "GPU 1 Utilization was 95% at 2:00 PM."* Metrics are cheap to store. You use them to build dashboards and trigger alerts. (Tool: Prometheus / Grafana / DCGM).
2. **Logs (The "Why"):** These are text messages generated by code when something happens. *Example: "ERROR: CUDA Out of Memory on Line 42."* Logs are expensive to store. You use them to read the exact error message after an alert fires. (Tool: ElasticSearch / Splunk / `dmesg`).
3. **Traces (The "Where"):** These track a single request as it jumps across multiple servers and functions. *Example: "The request spent 20ms in the API Gateway, 5ms in the CPU Tokenizer, and 400ms waiting for the GPU."* Traces are extremely heavy. You use them when a user says "the app is slow," but there are no errors in the logs and the metrics look fine. (Tool: Jaeger / Nsight Systems).

If your cluster only has Metrics (Grafana), you will know the GPU is broken, but you won't know why. If you only have Logs, you will know why it broke, but you won't get an alert until it's too late. A Senior Engineer implements all three.
...
To successfully observe an AI Factory, you must implement all three pillars of observability. Relying entirely on Grafana Dashboards (Metrics) will leave you blind to the root causes hidden in driver events (Logs), and blind to the microsecond-level synchronization bottlenecks in PyTorch (Traces).

```mermaid
flowchart TD
    subgraph The_Three_Pillars["The 3 Pillars of AI Observability"]
        direction LR
        
        subgraph Metrics["1. Metrics (The WHAT)"]
            M1[DCGM Exporter] --> M2[Prometheus / Grafana]
            M3[Use: Dashboards & Alerts]
        end
        
        subgraph Logs["2. Logs (The WHY)"]
            L1[dmesg / Fluentd] --> L2[ElasticSearch / Loki]
            L3[Use: Root Cause Analysis]
        end
        
        subgraph Traces["3. Traces (The WHERE)"]
            T1[PyTorch Profiler / Nsight] --> T2[Trace Viewer]
            T3[Use: Performance Tuning]
        end
    end
```
