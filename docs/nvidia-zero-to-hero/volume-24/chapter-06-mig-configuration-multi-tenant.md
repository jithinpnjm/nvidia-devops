...
You have **one H100 GPU** (80 GB HBM, 528 TensorCore TFLOPS, ~2 TB/s memory bandwidth). You must partition it so all three workloads meet their SLOs simultaneously.
...
```
Profile       Compute Units   GPU Memory   FP8 Throughput
─────────────────────────────────────────────────────────
7g.40gb       3/14 SMs        40 GB        75 TFLOPS
7g.20gb       2/14 SMs        20 GB        50 TFLOPS
7g.10gb       1/14 SMs        10 GB        25 TFLOPS
1g.10gb       1/14 SMs        10 GB        25 TFLOPS
1g.5gb        1/14 SMs        5 GB         25 TFLOPS
```

Only one profile per GPU (all partitions must be same size). Options:
- **7g.40gb (1 partition):** One workload uses full GPU
- **7g.20gb (2 partitions):** Two workloads share GPU
- **7g.10gb (4 partitions):** Four workloads (limited by SM count)
- **1g.10gb or 1g.5gb:** Time-sliced mode (not allowed here)
...
class MIGConfig:
    ...
    def create_partitions(self, profile):
        ...
        # Example profiles: 7g.40gb, 7g.20gb, 7g.10gb
        # Create partitions
        subprocess.run([
            'nvidia-smi', 'mig', '-cgi', '9,9,9,9'  # Create 4 instances of profile 9
        ], check=True)
...
# Example: Configure H100 with 2×7g.20gb partitions
if __name__ == '__main__':
    ...
    # Create 2 partitions (7g.20gb each)
    # (Details depend on your MIG profile setup)
...
|      ID     ID    Name         /PID       |
+-------------------------------------------+
| 0    1      0     7g.20gb      -          |  ← Batch inference partition
| 0    2      0     7g.20gb      -          |  ← Online inference partition
+-------------------------------------------+
...
```mermaid
flowchart TD
    A["Calculate resource needs per workload<br/>Batch: 50 img/s, p99<50ms<br/>Online: 5000 req/s, p99<5ms<br/>Training: 100 samples/s"] --> B["Estimate compute needed per workload<br/>Batch CNN: 5 TFLOPS sustained<br/>Online embed: 1 TFLOPS sustained<br/>Training: 20 TFLOPS sustained"]
    B --> C["Total: 26 TFLOPS needed<br/>H100 has 528 TFLOPS (50× excess)"]
    C --> D["Not compute-limited; check memory"]
    D --> E["Memory needs<br/>Batch CNN: 4GB<br/>Online embed: 2GB<br/>Training: 8GB<br/>Total: 14GB"]
    E --> F["MIG partition options?<br/>Can't fit 3 workloads in one GPU"]
    F --> G["Use 2×7g.20gb partitions<br/>Batch CNN on partition 1<br/>Online embedding on partition 2"]
    G --> H["Where does training go?<br/>Option 1: Time-slice one partition<br/>Option 2: Different GPU<br/>Option 3: Reduce training batch size"]
```
...
- Allocated: 7g.20gb partition (50 TFLOPS, ~40 GB/s memory) ✓ sufficient
...
- Allocated: 7g.20gb partition (severe overkill, but necessary for isolation) ✓
...
- Allocated: 7g.20gb partition (20 GB) ✓

### Step 2: Select MIG Configuration

Given 80 GB H100 and three workloads needing 14–20 GB each, only 2 can fit simultaneously on MIG:

**Configuration: 2×7g.20gb**
- Partition 1: Batch inference (CNN)
- Partition 2: Online inference (embedding)
- Training: Runs on different GPU or time-sliced (violates constraint; need third GPU)

**Alternative (if only one H100 available):**
- Use time-slicing (MPS mode) on one partition, MIG on another
- Or reduce training batch size to fit on 5 GB (not viable for good convergence)
- Or exclude training job (outside scope of this problem)
...
# Create 2 instances of 7g.20gb profile
...
Compute-wise, I have 500+ TFLOPS available, so all three could run concurrently on time-slicing. But the problem specifies no time-slicing (strict isolation).

With MIG, I can partition into slices. H100 supports 7g.20gb (2 partitions) or 7g.10gb (4 partitions). If I do 2×7g.20gb, I can fit batch + online on the H100, but training doesn't fit (need its own GPU or reduce batch size).

So the practical solution: 2×7g.20gb, assign batch to partition 1, online to partition 2. Training either goes on a second GPU or gets time-sliced separately.
...
**A:** "Two batch jobs are similar workloads—both can tolerate higher latency. I could run both on 7g.20gb partitions without interference, since they're not competing on latency SLOs.
...
4. **Document tradeoffs:** Why did you choose 2×7g.20gb over 4×7g.10gb? Trade-offs matter.
5. **Measure both ways:** Alone (baseline) and together (under contention).

## Discussion Questions

1. If batch inference needed 70 img/s (not 50), would you still use 7g.20gb for it?
