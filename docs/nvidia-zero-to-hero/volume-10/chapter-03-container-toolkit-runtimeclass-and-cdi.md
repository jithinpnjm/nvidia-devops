...
## Beginner's Primer: Docker, containerd, and CDI

Historically, running GPUs in containers was a messy hack. 
In the early days, you had to use a custom fork of Docker called `nvidia-docker`. This was terrible for production because you had to maintain a totally separate container engine just for AI workloads. 

Over time, this evolved. The AI industry moved away from Docker and standardized on **containerd** and **CRI-O**. NVIDIA adapted by creating the **NVIDIA Container Toolkit**, which acts as a wrapper around the standard `runc` (the low-level tool that actually creates containers). When Kubernetes created a Pod, it would use a feature called `RuntimeClass` to tell the cluster: *"Hey, use the NVIDIA wrapper for this Pod so the GPU gets injected."*

Today, the industry has evolved again to something much cleaner: **CDI (Container Device Interface)**. 
CDI is a vendor-neutral standard built directly into modern container runtimes. Instead of NVIDIA wrapping the runtime with custom code, CDI simply uses a JSON file. 
The device plugin looks at the host and generates a JSON file saying, *"Here are the exact device paths (like /dev/nvidia0) and libraries needed."* 
Containerd reads the JSON file and natively injects the GPU into the container. No custom runtime wrapper required. 

This chapter explains this critical boundary: how a GPU goes from being a physical piece of hardware on the server, to being a usable device inside an isolated Linux container namespace.
...
Turning a Kubernetes scheduling decision into a functional AI sandbox requires a strict runtime boundary. The Device Plugin handles allocation, while the NVIDIA Container Toolkit (using RuntimeClass or CDI) modifies the container specification to inject the physical device nodes and host libraries directly into the isolated Pod namespace. 

```mermaid
flowchart TD
    subgraph Kubelet["Node Kubelet"]
        DP[NVIDIA Device Plugin]
    end

    subgraph Runtime["Container Runtime (containerd)"]
        direction TB
        Create[Pod Create Request]
        CDI_JSON[CDI JSON Spec <br/> Generated by Device Plugin]
    ...
    subgraph Container["Target Pod Sandbox"]
        Libs[Mounted Host Driver Libs <br/> libcuda.so]
        Dev[Mounted Device Files <br/> /dev/nvidia0]
        App[CUDA Workload]
