...
- Default (300W) much higher than current (200W) → limit was changed
...
# GPU   Pwr Temp SM Mem  Enc Dec XSM Mxm Fbg Xid Pid Name
     0  300  65  95  75   45   0   0   0   0   0   - python
     1  145  60  45  40   20   0   0   0   0   0   - python
     2  280  62  90  70   42   0   0   0   0   0   - python
     3  295  63  93  72   44   0   0   0   0   0   - python
```

**Observation:**
- GPU 0, 2, 3: Power draw 280-300W (normal)
...
1. **Reduce GPU power draw to avoid cascading failure:**
   ...
   sudo nvidia-smi -i 1 -pl 280  # From 300W to 280W
   ```
...
1. **Calculate peak power demand:**
   ```bash
   # Per GPU max: 300W (A100 PCIe 80GB) or 700W (H100 SXM5)
   # PSU overhead: 30%
   # For 4x A100: 4 * 300 * 1.3 = 1560W min PSU
   
   # Check current PSU capacity
   sudo dmidecode --type 39 | grep "Nominal"
   # Expected: >= 1560W for 4x A100
   ```
...
1. **Power draw stable at expected level:**
   ...
   # Expected: Stable at ~280-300W (A100 PCIe 80GB) under load
   ```
...
|---------|----------|-----------|-----|--------------|
| Power limit drops 300W → 200W mid-training | nvidia-smi shows "Enforced Power Limit: 200W", dmesg shows "Power capping" | PSU insufficient capacity or voltage sag | Check PSU voltage with ipmitool; if &lt; 11.4V, upgrade PSU or redistribute load | Voltage stable at 12V ± 0.5V, power limit returns to 300W |
| Performance oscillates 300K → 100K samples/sec | Power draw oscillates 300W → 200W → 300W, Xid errors spike during low-power phase | Automatic power throttling loop (GPU tries to run full load, triggers capping, recovers, repeat) | Reduce GPU power limit permanently to stable 260W, or add second PSU to increase headroom | Performance stable at 250-300K samples/sec without oscillation |
| Single GPU slow while others fast | One GPU power capped at 200W while others at 300W, Xid errors on slow GPU | Power delivery failed for one GPU (faulty cable or connector) or PSU channel failing | Reseat power cable firmly; if issue persists, swap PSU channels or replace PSU | All GPUs show uniform power draw and performance |
