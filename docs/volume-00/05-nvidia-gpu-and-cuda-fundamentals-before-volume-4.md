---
title: "5 - NVIDIA GPU and CUDA fundamentals: what you need before Volume 4"
slug: "5-nvidia-gpu-and-cuda-fundamentals-before-volume-4"
sidebar_position: 5
description: "NVIDIA GPU and CUDA fundamentals: what you need before Volume 4 — Foundations Primer."
source_document: "Authored directly for the Foundations Primer — no DOCX source."
---

## What this chapter is, and what it isn't

This chapter will not make you a CUDA programmer or a GPU performance-tuning expert — Volume 4 (GPU and Accelerated Computing Foundations) covers that ground, and does so assuming you already know what a GPU fundamentally is and why it exists. What this chapter gives you is that "why" and "what," at a conceptual level, plus the driver/toolkit layering that trips up a lot of otherwise-experienced engineers. If you finish this chapter able to explain, in your own words, why a CPU isn't the right tool for certain jobs and what a GPU is doing instead, you're ready for Volume 4.
...
```
   CPU: [core][core][core][core]      -- few, powerful, flexible, sequential-ish
   GPU: [c][c][c][c][c][c][c][c]...   -- thousands, simpler, all doing the same op at once
        [c][c][c][c][c][c][c][c]...
```
...
```
[ GPU hardware ] -> [ Driver ] -> [ CUDA toolkit / runtime ] -> [ Your application ]
```

Each layer depends on the one below it being present and compatible. This is worth sitting with because Volume 4 spends real time on exactly this stack — specifically on why a driver version, a CUDA toolkit version, and an application's expectations all have to line up, and what goes wrong (and how you'd diagnose it) when they don't. If you walk into Volume 4 already knowing these are four separate, stacked things rather than three names for the same thing, its version-compatibility material will make sense on the first read.
...
## You're ready for Volume 4 when you can...
