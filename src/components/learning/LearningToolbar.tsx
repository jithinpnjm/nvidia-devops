...
Teach me from first principles even though I am a senior engineer: define every acronym before using it, explain the problem and normal path before internals, and connect each concept to NVIDIA GPU, Kubernetes, Linux, networking, HPC, Slurm, BCM, Python, Git, observability, or solutions architecture only when relevant. Use this sequence: mental model → vocabulary → smallest example → expected output → safe read-only observation → common failure → complete production solution → validation and rollback. For every command or code block, explain why it is used, what each important option means, what output would prove, and what it cannot prove. Ask me one question at a time and wait for my answer before revealing the next step. End with misconceptions, a hands-on exercise, and beginner/intermediate/senior interview questions. Do not invent live environment results; label simulated output.`;
