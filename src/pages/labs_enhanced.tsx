...
const labs: PythonExercise[] = [
  {id:'gpu-temp',title:'1 · Classify a GPU temperature reading',prompt:'Write a function that looks at one GPU temperature and says whether it is normal, warm, or hot. This is the simplest possible shape of an operations function: read one number, make a decision, return a word.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Check the lowest threshold first with if, then the middle range with elif, then fall back to hot with else.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Start a counter at 0 before the loop, and add 1 to it every time you see a healthy status inside the loop.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
assert find_faults({}) == []
print('PASS')`,hint:'Use fleet.items() in your for loop to get both the name and the status on each pass, then check the status.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
def find_faults(fleet):
  ...
  {id:'lowercase-gpus',title:'4 · Convert a list of GPU names to lowercase, safely',prompt:'Given a list of GPU names with inconsistent capitalization, return a new list where every name is lowercase. Inconsistent casing is a very common source of bugs when matching names across systems.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Create an empty list before the loop, then for each name call .lower() on it and append the result to that list.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Calling .split() with no arguments splits on any whitespace and gives you a list of three pieces in order.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Put the division inside a try block, and catch ZeroDivisionError to return 0.0 instead of letting it crash your program.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Use re.fullmatch with named groups; a parser should either return a complete record or fail clearly.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
def parse_log(line):
  ...
  {id:'retry',title:'8 · Retry only transient API failures',prompt:'Produce a deterministic retry decision for an API client. A retry without a deadline or classification can amplify an outage.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Separate classification from sleeping and I/O. 401/403/404 require a decision, not an automatic retry.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Ignore blank lines; split each row into exactly three stripped fields.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
def parse_inventory(text):
  ...
  {id:'oom',title:'10 · Detect cgroup memory risk',prompt:'Classify containers before an OOMKill. Use the limit—not node free memory—as the container safety boundary.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Compute the ratio once. Treat a missing/non-positive limit as invalid input.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
def memory_risk(working_set_mib, limit_mib):
  ...
  {id:'scheduling',title:'11 · Explain an unschedulable GPU Pod',prompt:'Turn scheduler event fragments into a ranked, human-readable diagnosis. Prefer specific constraints over a generic “Pending” message.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Map known fragments to explanations, then preserve first-seen order while removing duplicates.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
def scheduling_diagnosis(events):
  ...
  {id:'prometheus',title:'12 · Guard a Prometheus query',prompt:'Reject a dangerous unbounded query shape before it becomes a high-cardinality incident.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'This deliberately small guard is a teaching aid, not a PromQL parser.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
def query_guard(query):
  ...
  {id:'runbook',title:'13 · Turn evidence into a safe action',prompt:'Choose the least-destructive action from an incident state. Do not restart a node before preserving the evidence needed to prove the cause.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Order by containment first, then diagnosis; a repeated Xid is a hardware/driver safety signal.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
def next_action(node_ready, disk_pressure, xid_seen):
  ...
  {id:'capacity',title:'14 · Size a GPU capacity buffer',prompt:'Calculate a simple allocatable capacity target from demand, failure-domain reserve, and operational headroom.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Normalize stderr to lowercase. Return useful detail without hiding the original failure.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
def classify_command(returncode, stdout, stderr):
  ...
  {id:'linux-load',title:'16 · Diagnose Linux load without guessing',prompt:'Interpret a vmstat-style snapshot. High load is not automatically high CPU: runnable work and uninterruptible I/O need different actions.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Use run_queue versus CPU count for CPU pressure; use blocked tasks plus iowait for I/O pressure.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
def diagnose_load(load1, cpus, run_queue, blocked, iowait_pct):
  ...
  {id:'xid-correlation',title:'17 · Correlate GPU Xid events',prompt:'Group kernel Xid events by GPU UUID and identify repeated offenders without pretending every Xid has the same remediation.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
def repeated_gpu_faults(events, threshold=2):
  ...
  {id:'nccl-ranks',title:'18 · Find a distributed-training straggler',prompt:'Use per-rank step durations to identify a statistically meaningful outlier before blaming NCCL or the network.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
def straggler_ranks(step_ms, tolerance=1.20):
  ...
  {id:'inference-slo',title:'19 · Separate TTFT from decode regression',prompt:'Classify an inference SLO regression using queue, prefill, and decode signals instead of one average latency metric.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Treat more than 1.5× baseline as regressed. TTFT plus queue points toward admission/prefill.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
def inference_bottleneck(ttft_ms, tpot_ms, queue_ms, baseline):
  ...
  {id:'reconcile',title:'20 · Plan a Kubernetes reconciliation',prompt:'Compare desired and observed replica state and emit an idempotent plan rather than imperative trial-and-error.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Do not create/delete while termination is in flight; a real controller requeues and observes again.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
def reconciliation_plan(desired, ready, terminating):
  ...
  {id:'retry-storm',title:'21 · Detect an API retry storm',prompt:'Analyze client retry telemetry and flag amplification before a struggling dependency is overwhelmed.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'attempts includes original requests. A high attempts/requests ratio plus substantial error rate is dangerous.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
def retry_storm(requests, attempts, error_rate, clients):
  ...
  {id:'timeline',title:'22 · Build an incident timeline',prompt:'Merge changes, alerts, and symptoms into a stable chronological timeline while preserving source and correlation IDs.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Return a new list; do not mutate source records. Validate required fields.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Decode discrete sensors by exact bitmap match, never by numeric comparison; threshold sensors compare reading against unc/ucr.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
def node_health(sensors):
  ...
  {id:'firmware-drift',title:'24 · Detect firmware baseline drift across a fleet',prompt:'Compare installed component versions against the qualified baseline manifest before a coordinated change window opens. A node one revision behind is a normal backlog item; three revisions behind on BMC firmware is a blocker.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Compare dotted version strings as tuples of ints, not lexicographically, and skip components absent from the baseline.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Normalize each diff (e.g. split and sort its key=value pairs) before comparing runs for sameness; a reordered diff is not a new diff.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Score each action by its worst-case symbol, then take the maximum across the whole plan; do not average.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Compute a usage/allocated ratio per day; sustained means every ratio crosses the threshold, a burst means only the max does.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Parse each line with a strict regex into (rank, node) pairs, build a set of seen ranks per node, then set-subtract from expected.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Check the most fundamental prerequisite first: an unset visibility env var makes CDI and mount configuration irrelevant.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
def diagnose_gpu_visibility(config):
  ...
  {id:'canary-check',title:'30 · Check whether a canary wave actually represents the fleet',prompt:'A canary that passes every gate item is only evidence about the hardware/firmware combinations it contains. Find exactly which combinations in the fleet the proposed canary group does not cover before it ships to wave two.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Reduce both lists to sets of distinct combinations first; representativeness is a set-coverage question, not a count question.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Split into exactly five fields, convert status and latency to integers, count status >= 500, and track maximum latency.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
def summarize_access_logs(lines):
  ...
  {id:'latency-percentile',title:'32 · Calculate a latency percentile without hiding tail risk',prompt:'Implement nearest-rank percentile calculation so an SRE report can show p50, p95, and p99 instead of an average that hides slow requests.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
def percentile(values, percentile_value):
  ...
  {id:'alert-dedup',title:'33 · Deduplicate alerts into incidents',prompt:'Group repeated alerts by stable identity while retaining first seen, last seen, count, and highest severity. Do not deduplicate on volatile annotations.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Use a dictionary keyed by a tuple. Define an explicit severity ordering rather than comparing strings.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
def deduplicate_alerts(alerts):
  ...
  {id:'slo-burn',title:'34 · Calculate SLO error-budget burn rate',prompt:'Given good and total events plus an SLO target, calculate burn rate and classify whether the window is healthy, consuming budget, or paging-worthy.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Allowed error rate is 1 - SLO. Burn rate is observed error rate divided by allowed error rate.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
def error_budget_burn(good, total, slo):
  ...
  {id:'dependency-order',title:'35 · Compute a safe deployment order from dependencies',prompt:'Topologically order services so dependencies deploy before consumers, and detect a dependency cycle instead of producing a dangerous partial order.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Repeatedly select sorted nodes whose dependencies are already completed. If no node is ready while work remains, a cycle or missing dependency exists.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
def deployment_order(dependencies):
  ...
  {id:'config-precedence',title:'36 · Resolve configuration precedence with provenance',prompt:'Merge defaults, file configuration, environment variables, and CLI flags while recording which layer supplied every final value.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Apply layers from lowest to highest precedence. Update both value and source at the same time.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
def resolve_config(layers):
  ...
  {id:'circuit-breaker',title:'37 · Implement a circuit-breaker state transition',prompt:'Model closed, open, and half-open transitions from failures and elapsed cooldown. Keep policy separate from the network call.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Handle state-specific rules explicitly. An open breaker ignores failure count until cooldown permits one probe.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
def circuit_transition(state, consecutive_failures, cooldown_elapsed, probe_ok=None):
  ...
  {id:'token-bucket',title:'38 · Enforce a token-bucket rate limit',prompt:'Simulate a token bucket for timestamped requests and return which requests are admitted. This tests state transitions and burst-versus-sustained capacity reasoning.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Start full. Before each request, refill by elapsed × rate but cap at capacity; admit only when at least one token remains.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Track cumulative delay. Add the next capped exponential delay only when cumulative + delay is strictly below the deadline.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
def retry_schedule(base_seconds, max_delay, deadline):
  ...
  {id:'pod-capacity-fit',title:'40 · Test whether Kubernetes Pods fit allocatable capacity',prompt:'Calculate how many identical Pods fit a node using both CPU and memory requests, preserving system reserve. Identify the actual limiting resource.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Compute integer fit independently for CPU and memory; the smaller is capacity.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Compute expires - today. Use expired < 0, critical <= 14, warning <= 60, otherwise healthy; sort by days then name.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
def retained_backups(days, today):
  ...
  {id:'quorum-health',title:'44 · Distinguish healthy, degraded, and unsafe quorum',prompt:'Given replicas, reachable members, and writable members, classify a replicated control plane without assuming that one reachable node is safe.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Majority is replicas // 2 + 1. Healthy requires all reachable and exactly one writer; degraded still has majority and one writer.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
def quorum_health(replicas, reachable, writable):
  ...
  {id:'rollout-gate',title:'45 · Decide whether a canary deployment may advance',prompt:'Evaluate SLO, error-budget burn, sample size, and regression guardrails. A canary with no traffic is unknown—not successful.',starter:`from typing import List, Dict, Optional, Any, Tuple
...
print('PASS')`,hint:'Hold below the minimum sample. Roll back on burn >= 10, error rate > 2× baseline, or p95 > 1.5× baseline.',solution:`from typing import List, Dict, Optional, Any, Tuple
...
def rollout_decision(canary, baseline):
  ...
  {id:'async-retry',title:'46 · Async Exponential Backoff',prompt:'Write an async decorator or function that retries an unstable async network call (mocked here) up to 3 times, waiting 0.1s, 0.2s, then 0.4s. Synchronous `time.sleep()` would block the FastAPI event loop; you must use `asyncio.sleep`.',starter:`import asyncio
...
print('PASS')`,hint:'Inherit using `class AWSProvider(CloudProvider):`. Inside the class, define `def provision_node(self): return "AWS node"`. Do the same for GCP.',solution:`from typing import List, Dict, Optional, Any, Tuple
